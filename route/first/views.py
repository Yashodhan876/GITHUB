...
def random_route(l):
    route=[l[0]]
    ref_list=[i for i in l if i!=route[0]]
    for i in range(len(l)-1):
        min=geodesic(route[-1],ref_list[0]).km
        min_cord=ref_list[0]
        for j in ref_list:
            if min>geodesic(route[-1],j):
                min_cord=j
                min=geodesic(route[-1],j)
        ref_list.remove(min_cord)
        route.append(min_cord)
    return route
